...
export const ChatAssistant = ({ owner, repo, onClose }: ChatAssistantProps) => {
  const {
    analysis,
    loadingRepo,
    loadingAnalysis,
    ...
    analyzeCode,
  } = useGithubCodeAnalysis(owner, repo);
  ...
  useEffect(() => {
    if (owner && repo) {
      fetchRepositoryContents().catch(() => {
        // Error already handled in the hook
      });
    }
  }, [owner, repo, fetchRepositoryContents]);
        ...
        <CardContent className="p-4 space-y-4">
          {loadingRepo ? (
            ...
            <>
              ...
              >
                <p>
                  Hello! I'm your code assistant for {owner}/{repo}. Ask me
                  anything about your code, and I'll analyze it for you.
                </p>
              </div>
                  ...
                  <CardHeader className="pb-2 flex flex-row items-center justify-between">
                    ...
                    <span
                      className={`text-sm font-medium px-2 py-0.5 rounded ${
                        (analysis.security_score ?? 100) >= 80
                          ? "bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200"
                          : (analysis.security_score ?? 100) >= 50
                            ? "bg-amber-100 text-amber-800 dark:bg-amber-900 dark:text-amber-200"
                            : "bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200"
                      }`}
                    >
                      Score: {analysis.security_score ?? 0}/100
                    </span>
                  </CardHeader>
                      ...
                      analysis.vulnerabilities.length > 0 && (
                        <div>
                          <h5 className="text-sm font-medium text-destructive mb-1">
                            Potential Issues ({analysis.vulnerability_count ?? 0})
                          </h5>
